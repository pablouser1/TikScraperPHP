...
class Signer {
    ...
    function __construct(array $config) {
        ...
        if ($this->method === SignMethods::BROWSER) {
            $this->close_when_done = $config['close_when_done'] ?? true;
            $this->setupSelenium($this->url);
        }
    }

    function __destruct() {
        if ($this->method === SignMethods::BROWSER && $this->close_when_done) $this->driver->quit();
    }
     ...
     */
    private function browser(string $unsigned_url): object {
     ...
     */
    private function remote(string $unsigned_url): ?object {
    ...
    public function run(string $unsigned_url): ?object {
        ...
        switch ($this->method) {
            case SignMethods::BROWSER:
                $data = $this->browser($unsigned_url);
                break;
            case SignMethods::REMOTE:
                $data = $this->remote($unsigned_url);
                break;
        }
        return $data;
    }
